...
export default function Home() {
  ...
  const handleLogin = () => {
    authenticateUser(password, "senior");
  };
  ...
  const { mutate } = useUpdateAnonymous();
      ...
      {user &&
        data?.map((item) => (
          <MessageChat
            key={item?.id}
            data={item}
            onDelete={() => mutate({ ...item, flagged: true })}
            admin
          />
          // <p key={item?.id} onClick={() => mutate({ ...item, flagged: true })}>
          //   {item?.message} {item?.flagged && "flagged"}
          // </p>
        ))}
    </Wrapper>
  );
